...
def check_tensor(tensor, shape, throw=False):
    """Local shape check (replaces kaolin.utils.testing.check_tensor to avoid kaolin ABI issues with PyTorch 2.7+)."""
    if not torch.is_tensor(tensor):
        ok = False
    elif len(shape) != tensor.ndim:
        ok = False
    else:
        ok = all(s is None or s == d for s, d in zip(shape, tensor.shape))
    if not ok and throw:
        raise ValueError(f"Tensor shape {tuple(tensor.shape) if torch.is_tensor(tensor) else 'N/A'} does not match expected {shape}")
    return ok
