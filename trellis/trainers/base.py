...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.models = models
        self.dataset = dataset.train if hasattr(dataset, 'train') else dataset
        self.val_dataset = dataset.val if hasattr(dataset, 'val') else None     
    ...
    def prepare_dataloader(self, **kwargs):
        ...
        self.dataloader = DataLoader(
            ...
            # num_workers=int(np.ceil(os.cpu_count() // 2 - 1) // torch.cuda.device_count()),
            num_workers=8,
            # num_workers=0,
            pin_memory=True,
            drop_last=True,
    ...
    @abstractmethod
    def run_snapshot(self, num_samples, batch_size=4, verbose=False, mode='val', **kwargs):
        """
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=36, batch_size=4, verbose=False, mode='val'):
        """
        ...
        samples = self.run_snapshot(num_samples_per_process, batch_size=batch_size, verbose=verbose, mode=mode)
        ...
        if self.world_size > 1:
            for key in samples.keys():
                ...
                elif samples[key]['type'] == 'string':
                    all_strings = [None] * self.world_size
                    dist.all_gather_object(all_strings, samples[key]['value'])
                    if self.is_master:
                        if isinstance(samples[key]['value'], list):
                            flattened_strings = []
                            for string_list in all_strings:
                                if isinstance(string_list, list):
                                    flattened_strings.extend(string_list)
                                else:
                                    flattened_strings.append(string_list)
                            samples[key]['value'] = flattened_strings[:num_samples]
                        else:
                            samples[key]['value'] = all_strings

        # Save images and other data
        if self.is_master:
            os.makedirs(os.path.join(self.output_dir, 'samples', mode, suffix), exist_ok=True)
            for key in samples.keys():
                if samples[key]['type'] == 'image':
                    utils.save_image(
                        samples[key]['value'],
                        os.path.join(self.output_dir, 'samples', mode, suffix, f'{key}_{suffix}.jpg'),
                        nrow=int(np.sqrt(num_samples)),
                        normalize=True,
                        value_range=self.dataset.value_range,
                    )
                elif samples[key]['type'] == 'string':
                    txt_path = os.path.join(self.output_dir, 'samples', mode, suffix, f'{key}_{suffix}.txt')
                    try:
                        with open(txt_path, 'w', encoding='utf-8') as f:
                            value = samples[key]['value']
                            if isinstance(value, list):
                                for i, item in enumerate(value):
                                    f.write(f"Sample {i+1}: {str(item)}\n")
                            else:
                                f.write(str(value))
                    except Exception as e:
                        print(f"Error saving {key} to txt: {e}")
    ...
    def run(self):
        ...
        if self.step == 0:
            self.snapshot(suffix='init', mode='val')
            # pass
        else: # resume
        ...
        pbar = tqdm(total=self.max_steps, initial=self.step, dynamic_ncols=True, disable=not self.is_master)
        ...
        while self.step < self.max_steps:
            ...
            while True:
                try:
                    ...
                    if self.is_master:
                        pbar.write('Data iterator exhausted, restarting...')
                    self.data_iterator = cycle(self.dataloader)
                    continue
                except Exception as e:
                    if self.is_master:
                        pbar.write(f'Error loading data: {e}')
                    time.sleep(1)
                    continue
            ...
            # update progress bar
            if self.is_master:
                # simple description information
                pbar.set_description(f'Step {self.step}/{self.max_steps}')
                pbar.update(1)
                
                # detailed information still uses pbar.write
            ...
            if self.step % self.i_sample == 0:
                self.snapshot(mode='val')
                self.snapshot(mode='train')
        ...
        pbar.close()
        if self.is_master:
            self.snapshot(suffix='final', mode='val')
            self.snapshot(suffix='final', mode='train')
            self.writer.close()
