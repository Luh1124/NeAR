...
from ...utils.data_utils import cycle, BalancedResumableSampler, ResumableSampler
...
def _should_use_debug_safe_dataloader() -> bool:
    """
    Debugpy + multiprocessing workers can intermittently fail to attach and
    raise timeout errors in child processes. In debug sessions we fall back to
    single-process dataloading for stability.
    """
    force_debug_safe = os.getenv("TRELLIS_DEBUG_SAFE_DATALOADER", "").lower() in {"1", "true", "yes", "on"}
    if force_debug_safe:
        return True
    return sys.gettrace() is not None or os.getenv("DEBUGPY_LAUNCHER_PORT") is not None


def _dataloader_worker_config() -> tuple[int, bool]:
    if _should_use_debug_safe_dataloader():
        return 0, False
    num_workers = int(np.ceil(os.cpu_count() / torch.cuda.device_count()))
    return max(num_workers, 1), True
...
class SparseFlowMatchingTrainer(FlowMatchingTrainer):
    ...
    def prepare_dataloader(self, **kwargs):
        ...
        Prepare dataloader.
        """
        num_workers, persistent_workers = _dataloader_worker_config()
...
class SparseFlowMatchingS2ETrainer(FlowMatchingTrainer):
    """
    Trainer for sparse diffusion model with flow matching objective for S2E task.
    ...
    def training_losses(
        ...
        **kwargs
        ...
        pred = self.training_models['denoiser'](x_t, t * 1000, cond, x_1, **kwargs)
        assert pred.shape == noise.shape == x_0.shape
        target = self.get_v(x_0, noise, t)
        terms = edict()
        terms["mse"] = F.mse_loss(pred.feats, target.feats)
    ...
    def run_snapshot(
        ...
        sample_gt = sp.sparse_cat(sample_gt)
        sample_shaded = sp.sparse_cat(sample_shaded)
        sample = sp.sparse_cat(sample)
        sample_dict = {
            'sample_gt': {'value': sample_gt, 'type': 'sample'},
            'sample': {'value': sample, 'type': 'sample'},
            'sample_shaded': {'value': sample_shaded, 'type': 'sample'},
        }
        sample_dict.update(dict_reduce(cond_vis, None, {
            'value': lambda x: torch.cat(x, dim=0),
            'type': lambda x: x[0],
...
class SparseFlowMatchingCFGS2ETrainer(ClassifierFreeGuidanceMixin, SparseFlowMatchingS2ETrainer):
    """
    Trainer for sparse diffusion model with flow matching objective and classifier-free guidance.
