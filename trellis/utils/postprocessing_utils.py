...
def _fill_holes(
    ...
    """
    # Ensure verts is float32 to match view and projection matrices
    verts = verts.float() if verts.dtype != torch.float32 else verts
    ...
    radius = 2.0
    fov = torch.deg2rad(torch.tensor(40, dtype=torch.float32)).cuda()
...
def postprocess_mesh(
    ...
    if fill_holes:
        vertices, faces = torch.tensor(vertices, dtype=torch.float32).cuda(), torch.tensor(faces.astype(np.int32)).cuda()
